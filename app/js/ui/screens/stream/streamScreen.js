...
export const StreamScreen = {
  ...
  async mount(params = {}, navigationContext = {}) {
    ...
    if (this.boundClickContainer && this.boundClickContainer !== this.container && this.boundStreamClickHandler) {
      this.boundClickContainer.removeEventListener("click", this.boundStreamClickHandler);
    }
    if (!this.boundStreamClickHandler) {
      this.boundStreamClickHandler = (event) => {
        const actionTarget = event?.target?.closest?.(".focusable[data-action]");
    ...
    this.container.addEventListener("click", this.boundStreamClickHandler);
    this.boundClickContainer = this.container;
  ...
  cleanup() {
    ...
    if (this.boundClickContainer && this.boundStreamClickHandler) {
      this.boundClickContainer.removeEventListener("click", this.boundStreamClickHandler);
    }
    this.boundClickContainer = null;
